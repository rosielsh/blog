...
const components = {
    ...
    h1: ({ children, ...props }: HeadingProps) => {
        ...
        return (
            <h1 id={id} className="relative mb-4 mt-8 text-2xl font-bold group" {...props}>
                <span className="absolute -left-5 hidden text-gray-400 group-hover:inline">#</span>
                {children}
            </h1>
        );
    },
    ...
    h2: ({ children, ...props }: HeadingProps) => {
        ...
        const id = generateHeadingId(text);
        return (
            <h2 id={id} className="relative mb-4 mt-8 text-xl font-bold group" {...props}>
                <span className="absolute -left-4 hidden text-gray-400 group-hover:inline">#</span>
                {children}
            </h2>
        );
    },
    ...
    h3: ({ children, ...props }: HeadingProps) => {
        ...
        const id = generateHeadingId(text);
        return (
            <h3 id={id} className="relative mb-4 mt-6 text-lg font-bold group" {...props}>
                <span className="absolute -left-4 hidden text-gray-400 group-hover:inline">#</span>
                {children}
            </h3>
        );
    ...
    table: ({ children, ...props }: HTMLProps<"table">) => (
        <table
            className="my-6 w-full min-w-full divide-y divide-gray-200 border border-gray-300 dark:divide-gray-700 dark:border-gray-700"
            {...props}
        >
            {children}
        </table>
    ...
    thead: ({ children, ...props }: HTMLProps<"thead">) => <thead {...props}>{children}</thead>,

    tbody: ({ children, ...props }: HTMLProps<"tbody">) => (
        <tbody
    ...
    tr: ({ children, ...props }: HTMLProps<"tr">) => (
        <tr className="hover:bg-gray-50 dark:hover:bg-gray-800" {...props}>
            {children}
        </tr>
    ),

    th: ({ children, ...props }: HTMLProps<"th">) => (
        <th
    ...
    td: ({ children, ...props }: HTMLProps<"td">) => (
